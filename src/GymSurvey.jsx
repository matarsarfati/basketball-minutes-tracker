...
export default function GymSurvey() {
  ...
  // RPE Slider + 1-10 Chip Row (matches CourtSurvey/SurveyForm design)
  const RenderSlider = ({ label, value, onChange }) => {
    ...
    const GYM_EMOJI = ["😴", "🙂", "😊", "😌", "😰", "😕", "😣", "😫", "😬", "😱"];
    const emojiFor = (v) => GYM_EMOJI[v - 1] || "🙂";

    return (
      <div style={{ marginTop: 16 }} dir={lang === 'he' ? 'rtl' : 'ltr'}>
        <div style={{ fontWeight: 600, marginBottom: 4 }}>{label}</div>

        <div className="scaleSection" style={{ marginTop: 4 }}>
          {/* Gradient bar + invisible range input */}
          ...
          <div className="scaleSelected">
            ...
            <span style={{ marginLeft: 6, fontSize: 12, color: hasValue ? "#374151" : "#94a3b8" }}>
              {hasValue
                ? `${displayValue} — ${dynamicText}`
                : (lang === 'he' ? 'לא נבחר עדיין' : 'Selected: none yet')}
            </span>
          </div>
